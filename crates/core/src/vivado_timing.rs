...
fn parse_section<T, F: Fn(&[&str]) -> Option<T>>(txt: &str, marker: &str, row_fn: F) -> Vec<T> {
    let mut out = Vec::new();
...
pub fn parse_worst_endpoint(txt: &str) -> Option<FailingPath> {
    ...
    let start = lines
        .iter()
        .position(|l| l.trim_start().starts_with("Slack (VIOLATED)"))?;
