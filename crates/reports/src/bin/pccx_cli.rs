...
use pccx_core::{analyze_roofline, bottleneck::DetectorConfig, detect_bottlenecks};
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(s_idx) = args.iter().position(|a| a == "--source") {
        if s_idx + 1 < args.len() {
            ...
            for line in io::BufReader::new(s_file).lines() {
                let l = line.unwrap();
                println!("pccx% {}", l);
                if l.starts_with("report_utilization") {
                    println!("  => [CLI] Executing UtilReport...");
                } else if l.starts_with("generate_trace") {
                    println!("  => [CLI] Generating new pccx trace payload...");
                } else if l.starts_with("import_vcd") {
                    println!("  => [CLI] Bridging VCD database...");
    ...
    let bottleneck_ratio: f64 = args
        .windows(2)
        .find(|w| w[0] == "--bottleneck")
        .and_then(|w| w[1].parse().ok())
        .unwrap_or(0.5);
    let window_cycles: u64 = args
        .windows(2)
        .find(|w| w[0] == "--windows")
        .and_then(|w| w[1].parse().ok())
        .unwrap_or(256);
    let window_threshold: f64 = args
        .windows(2)
    ...
    println!(
        "  MAC Array     : {}×{}",
        pccx.header.arch.mac_dims.0, pccx.header.arch.mac_dims.1
    );
    ...
    if bottlenecks.is_empty() {
        ...
        println!(
            "  Detected {} high-occupancy DMA intervals:",
            bottlenecks.len()
        );
        for (i, ev) in bottlenecks.iter().take(5).enumerate() {
            println!(
                "  [{i}] Core={} Type={} Start={} Duration={}",
                ev.core_id.get(),
                ev.event_type,
                ev.start_cycle.get(),
                ev.duration.get()
    ...
    println!();
    println!(
        "── Windowed Bottleneck Detector ({} cyc, ≥{:.0}% share) ──",
        window_cycles,
        window_threshold * 100.0
    );
    let intervals = detect_bottlenecks(
        &trace,
        &DetectorConfig {
            window_cycles,
    ...
    if let Some(first) = trace.events.first() {
        ...
        println!("── First Event ─────────────────────────────────────────");
        println!(
            "  Core={} Type={} Start={} Duration={}",
            first.core_id.get(),
            first.event_type,
            first.start_cycle.get(),
            first.duration.get()
        );
