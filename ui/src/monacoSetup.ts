...
// ─── CX LSP provider registration ────────────────────────────────────────────

let cxProvidersRegistered = false;

/**
 * Registers Monaco HoverProvider, CompletionItemProvider, and a diagnostics
 * helper for the "cx" language.  Calls are forwarded to the Rust backend via
 * Tauri IPC (`cx_hover`, `cx_complete`, `cx_diagnostics_cmd`).
 * Idempotent — safe to call on every editor mount.
 */
export function registerCxProviders(monaco: typeof Monaco): void {
  if (cxProvidersRegistered) return;
  cxProvidersRegistered = true;

  // Hover provider
  monaco.languages.registerHoverProvider("cx", {
    async provideHover(
      model: Monaco.editor.ITextModel,
      position: Monaco.Position,
    ): Promise<Monaco.languages.Hover | null | undefined> {
      try {
        const result = await invoke<string | null>("cx_hover", {
          source: model.getValue(),
          line: position.lineNumber - 1,
  ...
  // Completion provider
  monaco.languages.registerCompletionItemProvider("cx", {
    triggerCharacters: ["l", "f", "i", "p", "s", "c", "_"],
    async provideCompletionItems(
      model: Monaco.editor.ITextModel,
      position: Monaco.Position,
    ): Promise<Monaco.languages.CompletionList> {
      try {
        const json = await invoke<string>("cx_complete", {
          source: model.getValue(),
          line: position.lineNumber - 1,
          col: position.column - 1,
        });
        const items: Array<{ label: string; detail: string | null; insertText: string }> =
          JSON.parse(json);
        const word = model.getWordUntilPosition(position);
        const range = new monaco.Range(
          position.lineNumber,
...
/**
 * Fetches CX diagnostics for the given model from the Rust backend and
 * pushes them into Monaco's marker system.  Call after edits (debounced).
 */
export async function updateCxDiagnostics(
  monaco: typeof Monaco,
  model: Monaco.editor.ITextModel,
): Promise<void> {
  try {
    const json = await invoke<string>("cx_diagnostics_cmd", {
      source: model.getValue(),
    });
    const items: LspDiagnosticItem[] = JSON.parse(json);
    const markers: Monaco.editor.IMarkerData[] = items.map((d) => ({
      startLineNumber: d.startLineNumber,
      startColumn: d.startColumn,
      endLineNumber: d.endLineNumber,
      ...
      severity: d.severity as Monaco.MarkerSeverity,
      message: d.message,
      source: d.source ?? "pccx-cx",
    }));
    monaco.editor.setModelMarkers(model, "pccx-cx", markers);
  } catch {
    monaco.editor.setModelMarkers(model, "pccx-cx", []);
  }
}
